...
TabuSearchSolution::TabuSearchSolution(
    ...
    std::vector<std::vector<double>> distanceMatrix, int n_tabu)
...
TabuSearchSolution::TabuSearchSolution(Solution s, int n_tabu) : Solution(s) {
  this->n_tabu_ = n_tabu;
...
inline bool TabuSearchSolution::Aspiration(double cost_increase,
...
void TabuSearchSolution::Solve() {
  ...
  for (int i = 0; i < n_tabu_; i++)
    tabu_list_queue_.push(std::vector<int>());
  while (c_it < max_it) {
    ++c_it;
    delta = 1 << 16;
    ...
    // TO check if solution found here is the same as the one found by the local
    // search if(delta>-0.00001) flag = true; if(flag && flag2){
    //   std::cout << "Local search solution found. Cost: " << best_cost_ << "
    //   Iterations: " << c_it << '\n'; flag2  = false;
    // }
    ...
    if (v_temp->id_ == v_temp_2->id_ && best_c < best_r) {
      ...
      v_temp_2->nodes_.insert(v_temp_2->nodes_.begin() + best_r + 1,
                              val_best_c);
