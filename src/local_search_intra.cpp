...
LocalSearchIntraSolution::LocalSearchIntraSolution(Solution s) : Solution(s) {
  ...
  for (auto &v : vehicles_) {
    while (true) {
      ...
      if (delta > -0.00001)
        ...
        if (best_c < best_r)
          v.nodes_.insert(v.nodes_.begin() + best_r, val_best_c);
        else
          v.nodes_.insert(v.nodes_.begin() + best_r + 1, val_best_c);
